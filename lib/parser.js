...
/**
 * Parses tasks and returns an abstract syntax tree (AST).
 *
 * @param {string} input The raw document to be parsed
...
/**
 * Transforms a TaskPaper AST `node` and adds TPS-ism's to it.
 *
 * @param {object} node The AST node
 * @param {*=} ctx Context to be passed around (since we work recursively)
 */
...
function transformTags (node, ctx) {
  ...
  node.infPoints = inferPoints(node)
  if (node.points === undefined) {
    node.points = node.infPoints
  } else {
    node.exPoints = node.points
  }

  node.infProgress = inferProgress(node)
  if (node.progress === undefined) {
    node.progress = node.infProgress
  } else {
    node.exProgress = node.progress
  }
