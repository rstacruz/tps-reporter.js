...
.item-bullet.-progress-1 > span {
  background: #D4E157;
}

.item-bullet.-progress-2 > span {
  background: #9CCC65;
}

.item-bullet.-progress-3 > span {
  background: #4CAF50;
}

.item-bullet.-done > span::before {
  content: '✓';
  font-size: 12px;
  font-weight: bold;
}
...
<td class='space'></td>
<td class='item'>
  <span class='item-bullet -progress-<%= Math.floor(node.progress * 3) %> <%= node.progress === 1 ? '-done' : '' %>'>
    <span style='width:<%= node.progress * 100 %>%'></span>
  </span>
  <%= node.value %>
</td>
<td class='progress'>
